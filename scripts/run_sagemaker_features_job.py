"""
Launch the Features SageMaker Processing job (reads splits from S3, writes feature CSVs to S3).

Run from project root:

    python scripts/run_sagemaker_features_job.py
...
FEATURES_OUTPUT_PATH = "/opt/ml/processing/output"
...
processor.run(
    ...
    outputs=[
        ProcessingOutput(
            source=FEATURES_OUTPUT_PATH,
            destination=S3_FEATURES_PREFIX,
            output_name="features",
        ),
    ],
    arguments=["-m", "src.features.feature_eng"],
    job_name=f"retail-features-job-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
)

print("Features job submitted. Check SageMaker console and S3 sagemaker/features/ prefix.")
