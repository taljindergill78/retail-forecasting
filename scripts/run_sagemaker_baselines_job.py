"""
Launch the Baselines SageMaker Processing job (reads splits from S3, writes baseline_results to S3).

Run from project root:

    python scripts/run_sagemaker_baselines_job.py
...
processor.run(
    ...
    outputs=[
        ProcessingOutput(
            ...
            destination=S3_BASELINES_PREFIX,
            output_name="baselines",
        )
    ],
    arguments=["-m", "src.model.run_baselines"],
    job_name=f"retail-baselines-job-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
)

print("Baselines job submitted. Check SageMaker console and S3 sagemaker/baselines/ prefix.")
